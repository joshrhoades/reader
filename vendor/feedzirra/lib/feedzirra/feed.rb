  ...
  class Feed
    ...
    def self.setup_easy curl, options
      ...
      if options[:max_redirects]
        curl.max_redirects = options[:max_redirects]
      else
      ...
      easy = Curl::Easy.new(url) do |curl|
        ...
        curl.on_success do |c|
          ...
          if klass
            ...
            err = NoParserAvailable.new("no valid parser for content.")# (this would unfortunately fail the whole 'multi', so it's not really usable)
            puts "No parser available: #{url}"
